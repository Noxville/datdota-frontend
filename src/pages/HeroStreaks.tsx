...
export default function HeroStreaks() {
  ...
  return (
    <div className={styles.page}>
      <PageMeta title="Hero Streaks — Pro Dota 2" description="Pro Dota 2 hero streaks — longest win/loss runs for individual heroes." />
      <div className={styles.header}>
        <h1>{isBest ? 'Best' : 'Worst'} Hero Streaks</h1>
        <p className={styles.subtitle}>
          {isBest
            ? 'Longest runs of consecutive wins on a single hero in tier 1–2 matches'
            : 'Longest runs of consecutive losses on a single hero in tier 1–2 matches'}
        </p>
      </div>

      <div className={styles.toggleRow}>
        <a
          href="/trivia/hero-streaks/best"
          className={`${styles.toggleBtn} ${isBest ? styles.toggleActive : ''}`}
        >
          Best
        </a>
        <a
          href="/trivia/hero-streaks/worst"
          className={`${styles.toggleBtn} ${!isBest ? styles.toggleActive : ''}`}
        >
          Worst
        </a>
