...
export default function EventKills() {
  const {
    filters,
    setFilters,
    clearFilters,
  ...
  const { data, isLoading, error } = useApiQuery<{ data: KillEvent[] }>(
    hasFilters ? '/api/events/hero-kills' : null,
    apiParams,
  )
  ...
  return (
    <div className={styles.page}>
      <PageMeta title="Hero Kills — Pro Dota 2 Stats" description="Hero kill events in pro Dota 2 — by location, time and player." />
      <div className={styles.header}>
        <h1>Hero Kills</h1>
        <p className={styles.subtitle}>
          Individual hero kill events in professional matches
        </p>
      </div>
      ...
      {isLoading && hasFilters && <EnigmaLoader text="Fetching kill events..." />}
