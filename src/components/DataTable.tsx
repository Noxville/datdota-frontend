    ...
    <div className={styles.tr} style={style} role="row">
      {row.getVisibleCells().map((cell) => {
        ...
        const rawValue = row.original
          ...
          : undefined
        const heatColor = getCellHeatColor(colId, rawValue, columnStats)
        const hasStats = colId in columnStats
        return (
          <div
            key={cell.id}
            className={`${styles.td} ${isNumeric ? styles.tdRight : ''}`}
            style={{ width: cell.column.getSize(), color: heatColor }}
            role="cell"
            onMouseEnter={
              hasStats && typeof rawValue === 'number'
                ? (e) => onCellEnter(e, colId, rawValue)
                : undefined
            }
...
export default function DataTable<T>({
  ...
  // Build a map of colId → tooltip label for headers
